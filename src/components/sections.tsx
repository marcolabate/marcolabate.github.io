...
function QaSnapshot(){
 const items=[
  {Icon:CalendarClock,label:'10+ years in QA'},
  {Icon:Gamepad2,label:'100+ PC/console titles tested'},
  {Icon:CodeXml,label:'Embedded DevQA experience'},
  {Icon:ListChecks,label:'Game QA, DevQA, UAT and SQL validation'},
  {Icon:Globe,label:'Remote international teams'},
  {Icon:Handshake,label:'Open to collaborations, consulting and long-term roles'}
 ];
 return <section aria-labelledby="qa-snapshot-heading" className="border-b border-line py-10 sm:py-12"><Container><Reveal><p className="text-sm font-semibold uppercase tracking-[.3em] text-cyan">At a glance</p><h2 id="qa-snapshot-heading" className="mt-3 text-2xl font-bold text-primary">QA snapshot</h2></Reveal><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{items.map(({Icon,label},i)=><Reveal key={label} delay={i*60} className="h-full"><div className="flex h-full items-center gap-3 rounded-2xl border border-line bg-card/70 px-4 py-3.5 backdrop-blur transition-[background-color,border-color] duration-300 hover:border-cyan/40 hover:bg-elevated/80"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-pillBorder bg-pill text-cyan"><Icon className="h-[18px] w-[18px]" aria-hidden="true"/></span><span className="text-sm font-medium leading-normal text-primary">{label}</span></div></Reveal>)}</div></Container></section>;
}
